...
def inbox(request):
    """
    One card per conversation using each convo’s latest visible message for the current user.
    Only shows messages received by the user (typical inbox behavior).
    """
    ...
    if query:
        ...
        for m in latest_messages:
            in_subject = q in (m.subject or "").lower()
            in_body = q in (m.body or "").lower()
            in_participants = any(
                q in (p.get_full_name() or p.username or "").lower()
                for p in m.conversation.participants.all()
            )
            if in_subject or in_body or in_participants:
                filtered.append(m)
        latest_messages = filtered
    ...
    return render(
        ...
        "messaging/messages.html",
        {
            ...
            "message_list": latest_messages,   # <— avoid 'messages' name collision
            "active_tab": "inbox",
...
def all_messages(request):
    ...
    return render(
        ...
        "messaging/messages.html",
        {
            "message_list": qs,              # <— avoid 'messages'
            "active_tab": "all",
            "unread_count": unread_count,
            "query": query,
        },
...
def message_detail(request, pk):
    message = get_object_or_404(
        Message.objects.select_related("conversation", "sender", "recipient"), pk=pk
    )
    ...
    # Hide archived for current user
    ...
    return render(
        ...
        {"message": message, "thread_messages": thread_messages},
    )


# ---------- Conversation entry (jump to latest visible) ----------
@login_required
...
def archive_message(request, pk):
    ...
    if m.conversation_id:
        return redirect("conversation_detail", pk=m.conversation_id)
    return redirect("messages")
...
# ---------- Delete (soft for user; hard when both deleted) ----------
@login_required
@require_POST
...
def download_attachment(request, pk):
    a = get_object_or_404(
        MessageAttachment.objects.select_related("message__sender", "message__recipient"), pk=pk
    )
    msg = a.message
